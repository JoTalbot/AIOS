...
class WeakToStrongGeneralization:
    ...
    def measure_generalization_gap(self, weak_labels: list[Any], strong_predictions: list[Any]) -> dict[str, Any]:
        ...
        agreement = sum(1 for w, s in zip(weak_labels, strong_predictions, strict=False) if w == s) / max(
            len(weak_labels), 1
        )
        return {
    ...
    def stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_generalization": round(
                sum(e.generalization_score for e in self._exp_records) / max(len(self._exp_records), 1),
                2,
            ),
        }
