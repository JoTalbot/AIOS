...
def data_dir(tmp_path):
    """Изолированное хранилище для каждого fintech-теста."""
    return str(tmp_path / "data")


def test_wallet_manager_4_way_split(data_dir):
    mgr = AIOSWalletManager(data_dir)
    transaction = mgr.record_income(100.0, source="test", task_id="test-income")
    assert transaction["share_per_wallet_usd"] == 25.0
...
def test_treasury_audit_and_buffer(data_dir):
    tm = AIOSTreasuryManager(data_dir)
...
def test_smart_liquidity_router_scan(data_dir):
    router = AIOSSmartLiquidityRouter(data_dir)
...
def test_fiat_dispatcher_live_rate(data_dir):
    dispatcher = AIOSFiatDispatcher(data_dir)
...
def test_kraken_client_ticker(data_dir):
    client = AIOSKrakenClient(data_dir)
    ticker_res = client.get_ticker("XXBTZUSD")
    assert ticker_res["status"] == "success"
    assert "XXBTZUSD" in ticker_res.get("ticker", {})


def test_dex_arbitrage_scanner(data_dir):
    scanner = AIOSDEXArbitrageScanner(data_dir)
