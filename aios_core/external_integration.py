...
class IntegrationMetrics:
    ...
    def __init__(self):
        ...
        self.message_queue_processed = MetricCounter("message_queue_processed", "Total messages processed")
        self.api_integration_latency = MetricHistogram("api_integration_latency", "API integration latency in ms")
...
class ExternalIntegrationAPI:
    ...
    async def get_integration_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "message_queue": {
                name: {
                    "processed": connector.metrics.message_queue_processed.value
                }
                for name, connector in self.message_queues.items()
            },
            "latency": {
                "avg": self.metrics.api_integration_latency.values[-1] if self.metrics.api_integration_latency.values else 0,
                "max": max(self.metrics.api_integration_latency.values) if self.metrics.api_integration_latency.values else 0
            }
        }
