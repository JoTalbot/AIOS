...
class ExternalIntegrationManager:
    ...
    async def process_event(self, event: IntegrationEvent):
        ...
        try:
            with tracer.span("integration_event_processing", {
...
class ExternalIntegrationAPI:
    ...
    def create_enhanced_app(self) -> Starlette:
        ...
        integration_routes = [
            ...
            Route("/api/v1/integrations/webhooks/{endpoint}", self._webhook_register, methods=["POST"]),
            Route("/api/v1/integrations/webhooks/{endpoint}", self._webhook_unregister, methods=["DELETE"]),
            ...
            Route("/api/v1/integrations/sync/external", self._external_sync, methods=["POST"]),
            Route("/api/v1/integrations/sync/status", self._sync_status),
            
            # Webhook testing
            Route("/api/v1/integrations/webhooks/test", self._webhook_test, methods=["POST"]),
        ]
