...
class Planner:
    ...
    def _llm_json(self, prompt: str) -> dict:
        system = (
            ...
            "ФИО, телефон, город, отделение Новой Пошты (или способ доставки). Спрашивай об этом "
            "уже при первом контакте/интересе к товару, а не только при оформлении покупки. "
            "Если данных нет — в params.text задай вежливый вопрос клиенту, чтобы их получить."
        )
    ...
    def _propose_customer(self, platform: str, chat: str, text: str, intent: dict,
        ...
        prompt = (
            ...
            "ДАННЫЕ ДЛЯ ОТПРАВКИ: если у клиента ещё НЕТ полных данных (ФИО, телефон, город, "
            "отделение/способ доставки) — задай ему вежливый вопрос в params.text, чтобы получить их "
            "СЕЙЧАС, до оформления отправки. Не оставляй отправку без этих данных.\n"
