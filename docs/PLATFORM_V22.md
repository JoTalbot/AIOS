...
- [x] White-label groundwork ✅ `aios_core/whitelabel_ads.py` — tenant-конфиги брендов (компания/телефон/футер/наценка/язык), изолированные черновики объявлений (изоляция по tenant_id + дневная квота, переживает рестарт), цена = base × (1+markup%); CLI: create/draft/drafts; live-публикация в аккаунт клиента — следующий шаг за approve
