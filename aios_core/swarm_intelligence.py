...
class Ant:
    ...
    def construct_path(self, pheromone: list[list[float]], distance: list[list[float]], alpha: float = 1.0, beta: float = 2.0) -> list[int]:
...
class ParticleSwarmOptimizer:
    ...
    def __init__(self, num_particles: int = 20, dimensions: int = 5) -> None:
        self.particles: list[Particle] = [Particle([random.uniform(-10, 10) for _ in range(dimensions)]) for _ in range(num_particles)]
    ...
    def ant_colony_optimize(self, num_nodes: int = 10, num_ants: int = 5, iterations: int = 50) -> dict[str, Any]:
        ...
        return {"best_path": best_path, "best_cost": round(best_cost, 2), "nodes": num_nodes}

    def convergence_report(self) -> dict[str, Any]:
        """Report convergence history."""
        if not self._convergence_history:
            return {"status": "no_data"}
        return {"iterations": len(self._convergence_history), "initial": self._convergence_history[0], "final": self._convergence_history[-1], "improvement": round(self._convergence_history[0] - self._convergence_history[-1], 4)}
