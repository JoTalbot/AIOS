...
async def new_cell_and_run(page, code):
    ...
    await page.mouse.click(400,300)   # проверенный рабочий способ
...
async def main():
    ...
    # ищем реальный ВЫВОД (не код) - строки после последнего вхождения команд
    for kw in ["MODELS_SRC","UPLOAD_OK","UPLOAD_ERR","UPLOAD_DONE"]:
        i=body.rfind(kw)
        if i>=0:
            print("==%s=="%kw, body[max(0,i-5):i+130].replace("\xa0"," ").replace("\n"," ")[:170])
    await p.stop()
asyncio.run(main())
