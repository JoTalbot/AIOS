...
class OLXHandlersMixin:
    ...
    async def _olx_competitive(self, request: Request) -> JSONResponse:
        ...
        if not fingerprint:
            return JSONResponse({"error": "query parameter 'fingerprint' is required"}, status_code=400)
    ...
    async def _olx_competitive_refresh(self, request: Request) -> JSONResponse:
        ...
        try:
            ...
            (await request.json() if (request.headers.get("content-length") or "0") != "0" else {})
    ...
    async def _olx_own_stagnant(self, request: Request) -> JSONResponse:
        ...
        items = OwnAdsTracker(self._olx_db(request)).stagnant(min_age_days=min_age, min_views_per_day=min_rate)
        return JSONResponse({"count": len(items), "items": items})
    ...
    async def _olx_own_snapshot(self, request: Request) -> JSONResponse:
        ...
        try:
            ...
            result = OwnAdsTracker(self._olx_db(request)).record_snapshot(ads, seen_at=body.get("seen_at"))
            return JSONResponse(result)
        except Exception as exc:
            return JSONResponse({"error": str(exc)}, status_code=400)
