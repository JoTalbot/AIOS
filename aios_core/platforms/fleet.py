...
def default_start_emulator(avd_name: str) -> None:
    ...
    subprocess.Popen(
        f"emulator @{avd_name} -no-window -no-audio -gpu swiftshader_indirect",
        shell=True,
...
def ensure_device(
    ...
    profile_store=None,
    avd_prefix: str = "aios",
    create_avd: Callable[[str], bool] | None = None,
    start_emulator: Callable[[str], None] | None = None,
    wait_serial: Callable[[list[str]], str | None] | None = None,
