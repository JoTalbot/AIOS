...
Данный документ содержит подробные инструкции по развертыванию инфраструктуры **AIOS** (в том числе в "1 клик", SSH-деплой на сервер и Kubernetes), а также детальное руководство по работе с мобильной роботизацией (RPA) для **Android приложений**.

---

## 🖥️ Развертывание на SSH-сервер (VPS / Dedicated)

Подробное пошаговое руководство вынесено в отдельный документ: **[DEPLOY_SSH_RU.md](DEPLOY_SSH_RU.md)**.

### Быстрый запуск деплоя по SSH:
1. **Через GitHub Actions**: Добавьте секреты `SSH_HOST`, `SSH_USER`, `SSH_KEY`, `SSH_PORT`, `SSH_PATH` в репозитории GitHub. Деплой выполняется автоматически при push в ветку `main`.
2. **Через CLI скрипт**:
   ```bash
...
3. **Первичная подготовка сервера**:
   ```bash
   bash ./scripts/setup_remote_server.sh
   ```
