...
async def test_full_pipeline_happy_path(advisor):
    ...
    result = await advisor.process_incoming_message(
        ...
        context={"lang": "ru", "product": {"price": 10000}, "negotiation_history": []},
    )
...
async def test_pipeline_with_negative_sentiment(advisor):
    ...
    result = await advisor.process_incoming_message(
        message_id="msg_002", platform="olx", incoming_text="Это обман! Хочу возврат!", context={"lang": "ru"}
    )
...
async def test_pipeline_with_compliance_violation(advisor):
    ...
    advisor.template_engine.create_template(
        name="Гарантия", content="Гарантируем 100% оригинал!", intent="greeting", variables=[]
    )

    result = await advisor.process_incoming_message(
        message_id="msg_003", platform="olx", incoming_text="Здравствуйте", context={"lang": "ru"}
    )
