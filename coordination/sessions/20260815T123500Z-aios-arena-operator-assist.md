...
- 2026-08-15T13:30Z — контрольный прогон --no-ml-gate (6 мес): ИДЕНТИЧНЫЙ результат (-0.30%, 1 сделка, APT stop_loss). Блокировки: global_drawdown_kill 8086, rl_veto 3624, OK 1. Ключевой вывод: узкое место — НЕ ML-гейт, а kill-switch equity drawdown 0.25%: первая же сделка по SL (-1.49% на позицию 200 USD = -0.30% equity) превышает лимит и навсегда блокирует входы; дальше система 6 месяцев не торгует by design. ML-гейт в owner-профиле не добавляет и не убирает ценности. Отчёт: docs/TRADING_BACKTEST_6M_NO_ML_2026-08-15_RU.md.
## Ход работы и решения
...
- [PASS] health-check скрипты не мониторят ollama — ложных алертов не будет.
...
## Handoff
Чистка завершена (81%→46%). Открытый вопрос: chrome_twin (1.9G) — только после остановки автоматики Chrome и с согласия на потерю Google-сессии. Рекомендация: усилить retention aios-rotate-backups (отдельная задача).
## Handoff
