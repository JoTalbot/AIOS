...
class AgentScoreboard:
    ...
    def record(self, role: str, *, success: bool, iterations: int = 1, reviewer_rejected: bool = False, security_violation: bool = False) -> None:
        stat = self.stats.setdefault(role, AgentStats())
        stat.attempts += 1
        stat.total_iterations += max(1, iterations)
        if success:
            stat.successes += 1
        else:
            stat.failures += 1
    ...
    def score(self, role: str) -> float:
        ...
        if not stat or not stat.attempts:
            return 0.0
        penalty = min(0.5, stat.reviewer_rejections / stat.attempts * 0.25 + stat.security_violations / stat.attempts * 0.5)
        iteration_penalty = min(0.25, max(0.0, stat.avg_iterations - 1.0) * 0.1)
        return max(0.0, stat.success_rate - penalty - iteration_penalty)
