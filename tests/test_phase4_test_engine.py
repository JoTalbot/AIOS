...
class TestModels:
    ...
    def test_suite_result_category_breakdown(self):
        from aios_core.test_engine.models import TestSuiteResult, TestResult, TestStatus, TestCategory
...
class TestRunner:
    ...
    def test_runner_stats(self):
        ...
        runner.run_case(TestCase(
            name="s",
            category=TestCategory.CONSTITUTIONAL,
            action={"goal": "g", "scope": "s", "risk": "low", "audit_log": True, "agent_id": "a", "authority": "user"},
            expected_decision="ALLOW",
        ))
...
class TestSuites:
    ...
    def test_all_suites_return_test_cases(self):
        ...
        from aios_core.test_engine.models import TestCase
        for suite_fn in [constitutional_compliance_suite, security_policy_suite,
                        evolution_safety_suite, integration_suite]:
            cases = suite_fn()
...
class TestReporter:
    ...
    def test_generate_report_all_passed(self):
        ...
        from aios_core.test_engine.models import (
            TestSuiteResult, TestResult, TestStatus, TestCategory,
        )
        ...
        suite = TestSuiteResult(
            ...
            results=[
                TestResult(test_name="t1", status=TestStatus.PASSED,
                           category=TestCategory.CONSTITUTIONAL),
                TestResult(test_name="t2", status=TestStatus.PASSED,
                           category=TestCategory.CONSTITUTIONAL),
                TestResult(test_name="t3", status=TestStatus.PASSED,
                           category=TestCategory.CONSTITUTIONAL),
            ],
        )
    ...
    def test_generate_report_with_failures(self):
        ...
        suite = TestSuiteResult(
            ...
            results=[
                TestResult(test_name="t1", status=TestStatus.PASSED,
                           category=TestCategory.CONSTITUTIONAL),
                TestResult(test_name="t2", status=TestStatus.FAILED,
                           category=TestCategory.CONSTITUTIONAL,
                           expected_decision="ALLOW", actual_decision="DENY",
                           message="Expected ALLOW, got DENY"),
                TestResult(test_name="t3", status=TestStatus.FAILED,
                           category=TestCategory.CONSTITUTIONAL,
                           expected_decision="ALLOW", actual_decision="DENY",
                           message="Expected ALLOW, got DENY"),
            ],
    ...
    def test_failures_text_with_failures(self):
        ...
        suite = TestSuiteResult(
            ...
            results=[
                TestResult(test_name="bad_test", status=TestStatus.FAILED,
                           category=TestCategory.CONSTITUTIONAL,
                           expected_decision="ALLOW", actual_decision="DENY",
                           message="Mismatch"),
            ],
    ...
    def test_report_overall_status_passed(self):
        from aios_core.test_engine.reporter import TestReporter
        from aios_core.test_engine.models import (
            TestSuiteResult, TestStatus, TestCategory, TestResult,
        )
        reporter = TestReporter()
        suite = TestSuiteResult(
            suite_name="s", status=TestStatus.PASSED,
            total=1, passed=1, failed=0, errors=0, skipped=0,
            results=[TestResult(test_name="t", status=TestStatus.PASSED,
                                category=TestCategory.CONSTITUTIONAL)],
        )
    ...
    def test_report_overall_status_partial(self):
        ...
        reporter = TestReporter()
        suite = TestSuiteResult(
            suite_name="s", status=TestStatus.FAILED,
...
class TestEngine:
    ...
    def test_run_case(self):
        ...
        from aios_core.test_engine.models import TestCase, TestCategory
        engine = TestEngine(CONSTITUTION_DIR, POLICIES_DIR)
        result = engine.run_case(TestCase(
            name="engine_case",
            category=TestCategory.CONSTITUTIONAL,
            action={
                "goal": "Read metrics",
