...
def test_hints_messenger_open_chats_deeplink_and_list(tmp_path):
    ...
    adb = _ADB([INBOX_XML])
    messenger = WhatsAppMessenger(adb=adb, storage=None,
...
def test_hints_messenger_uncalibrated_is_honest_empty():
    ...
    messenger = ViberMessenger(adb=_ADB([INBOX_XML]), storage=None,
                               messenger_hints={})
...
def test_bootstrap_doctor_reports_gaps(tmp_path, monkeypatch):
    ...
    report = WhatsAppBootstrap(
        ...
        directory=str(tmp_path), which=lambda n: "/usr/bin/adb" if n == "adb" else None,
    ).doctor()
...
def test_onboard_wizard_ready_report(tmp_path, monkeypatch, modules_path):
    ...
    feed.write_text(
        ...
        "resource-id=''/></node></hierarchy>", encoding="utf-8")
    calls = []
    ...
    report = onboard_package(
        apk=None, name="wizzdemo", package="com.wizz.demo",
        project_root=str(tmp_path), driver=driver, serial="emulator-5554",
        dry_run=False,
    )
...
def test_onboard_wizard_needs_device_honest(tmp_path, modules_path):
    ...
    report = onboard_package(
        apk=None, name="wizzneeds", package="com.wizz.needs",
        project_root=str(tmp_path), dry_run=False,
    )
    assert report["status"] != "ready"
    assert any("калибровка" in c or "--dump" in c
...
def test_cli_onboard_dry_run(tmp_path, capsys):
    ...
    main(["onboard", "--name", "wizzcli",
          "--package", "com.wizz.cli",
          "--root", str(tmp_path), "--no-fetch", "--dry-run"])
...
def test_cli_platforms_reels_tiktok(tmp_path, capsys, monkeypatch,
                                    registered):
    from aios_cli import main
    ...
    monkeypatch.setattr(
        adb_mod, "ADBController", lambda *a, **k: fake)
    ...
    main(["platforms", "reels", "--platform", "tiktok",
          "--db", str(db), "--max", "10", "--directory", str(tmp_path)])
    out = json.loads(capsys.readouterr().out)
    assert out["platform"] == "tiktok"
    assert out["new"] == 2 and out["cards"][0]["title"] == "TT1"


def test_cli_platforms_doctor_tiktok(tmp_path, capsys, monkeypatch,
                                     registered):
    from aios_cli import main
    _write_yaml(tmp_path, "tiktok", "com.zhiliaoapp.musically")
    monkeypatch.setenv("AIOS_PROFILES_DB", str(tmp_path / "profiles.sqlite"))
    from aios_core.platforms import ProfileStore
    ProfileStore.reset_default()
    main(["platforms", "doctor", "--platform", "tiktok",
...
def test_cli_whatsapp_dm_guarded(tmp_path, capsys, registered):
    ...
    main(["whatsapp", "dm-send", "--chat", "chat:anna",
          "--text", "Добрий день!", "--db", str(db)])
    out = json.loads(capsys.readouterr().out)
...
def test_rest_shard_jobs_plane(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        response = client.post("/api/v1/shards/jobs", json={
            "profile": "instagram:main", "kind": "autopilot",
            "payload": {"args": ["--max", "3"]}})
        assert response.status_code == 201
        assert response.json()["enqueued"] >= 1

        response = client.post(
            "/api/v1/shards/jobs", json={"profile": "x:y"})
