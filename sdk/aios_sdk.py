...
class AIOSClient:
    ...
    async def memory_store(self, content: dict, category: str = "operational", tags: Optional[List[str]] = None) -> dict:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            resp = await client.post(
    ...
    async def kg_add_node(self, label: str, node_type: str = "entity", properties: Optional[dict] = None) -> dict:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            resp = await client.post(
    ...
    async def marketplace_plugins(self, platform: str = "") -> dict:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            resp = await client.get(self._url("/api/v1/marketplace/plugins"), params={"platform": platform}, headers=self.headers)
            resp.raise_for_status()
            return resp.json()

    # --- AI Advisor (H3.11) ---

    async def advisor_draft_reply(self, platform: str, original_message: str, recipient: str, item_context: Optional[dict] = None) -> dict:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            resp = await client.post(
                self._url("/api/v1/advisor/draft"),
                json={"platform": platform, "original_message": original_message, "recipient": recipient, "item_context": item_context or {}},
                headers=self.headers
            )
            resp.raise_for_status()
            return resp.json()
    ...
    async def watch_events(self, on_event: Callable[[dict], None], channels: Optional[List[str]] = None):
...
class AIOSClientSync:
    ...
    def ready(self): return self._run(self._async.ready())
    def stats(self): return self._run(self._async.stats())
    ...
    def list_tasks(self): return self._run(self._async.list_tasks())
    def get_task(self, task_id: str): return self._run(self._async.get_task(task_id))
    def evaluate(self, action: dict): return self._run(self._async.evaluate(action))

    # Evolution
    def propose_evolution(self, *a, **kw): return self._run(self._async.propose_evolution(*a, **kw))
    def list_proposals(self): return self._run(self._async.list_proposals())

    # Memory
    def memory_store(self, *a, **kw): return self._run(self._async.memory_store(*a, **kw))
    def memory_search(self, *a, **kw): return self._run(self._async.memory_search(*a, **kw))

    # KG
    def kg_add_node(self, *a, **kw): return self._run(self._async.kg_add_node(*a, **kw))
    def kg_query(self, query: str): return self._run(self._async.kg_query(query))

    # Android
    def android_list_devices(self): return self._run(self._async.android_list_devices())
    def android_device_status(self, device_id: str): return self._run(self._async.android_device_status(device_id))
    def apps_list(self, platform: str = ""): return self._run(self._async.apps_list(platform))
    def apps_get(self, platform: str): return self._run(self._async.apps_get(platform))

    # Shards
    def shards_stats(self): return self._run(self._async.shards_stats())
    def shards_jobs(self): return self._run(self._async.shards_jobs())

    # Marketplace
    def marketplace_search(self, *a, **kw): return self._run(self._async.marketplace_search(*a, **kw))
    def marketplace_publish(self, *a, **kw): return self._run(self._async.marketplace_publish(*a, **kw))
    def marketplace_plugins(self, *a, **kw): return self._run(self._async.marketplace_plugins(*a, **kw))

    # Advisor
    def advisor_draft_reply(self, *a, **kw): return self._run(self._async.advisor_draft_reply(*a, **kw))
    def advisor_summarize_inbox(self, *a, **kw): return self._run(self._async.advisor_summarize_inbox(*a, **kw))
    def advisor_price_advice(self, *a, **kw): return self._run(self._async.advisor_price_advice(*a, **kw))
