...
class ScalableOversight:
    ...
    def __init__(self) -> None:
        self.techniques: list[str] = [
            "debate", "amplification", "recursive_reward_modeling",
            "honest_ai", "weak_to_strong_generalization",
        ]
        self.evaluations: list[dict[str, Any]] = []
        self._records: list[OversightRecord] = []
        self._cost_estimates: dict[str, float] = {
            "debate": 2.0, "amplification": 5.0, "recursive_reward_modeling": 3.0,
            "honest_ai": 1.0, "weak_to_strong_generalization": 4.0,
        }

    def debate(self, claim: str, agents: int = 2) -> dict[str, Any]:
        """Debate oversight (backward-compatible)."""
        confidence = round(random.uniform(0.8, 0.95), 2)
        result = {"claim": claim, "agents": agents, "winner": "agent_1", "confidence": confidence}
        self._records.append(OversightRecord("debate", quality=confidence, cost=agents * self._cost_estimates["debate"]))
        return result
    ...
    def weak_to_strong(self, weak_model: Any, strong_model: Any) -> dict[str, Any]:
        ...
        self._records.append(OversightRecord("weak_to_strong", quality=gen_score, cost=self._cost_estimates["weak_to_strong_generalization"]))
        return result
    ...
    def amplification_oversight(self, base_agent: Any, levels: int = 3) -> dict[str, Any]:
        ...
        self._records.append(OversightRecord("amplification", quality=quality, cost=levels * self._cost_estimates["amplification"]))
        return result
    ...
    def recursive_reward_oversight(self, iterations: int = 5) -> dict[str, Any]:
        ...
        result = {"iterations": iterations, "quality": quality, "technique": "recursive_reward"}
        self._records.append(OversightRecord("recursive_reward", quality=quality, cost=iterations * self._cost_estimates["recursive_reward_modeling"]))
        return result
    ...
    def cost_efficiency_ranking(self) -> list[dict[str, Any]]:
        ...
        for technique in self.techniques:
            ...
            avg_cost = sum(r.cost for r in records) / max(len(records), 1) if records else self._cost_estimates.get(technique, 1.0)
            ranking.append({"technique": technique, "quality": round(avg_quality, 2), "cost": round(avg_cost, 2), "efficiency": round(avg_quality / max(avg_cost, 0.1), 2)})
