...
class Kernel:
    def __init__(self, orchestrator=None):
        ...
        self.components = ComponentLifecycleManager(self.registry, self.events)
        self.orchestrator = orchestrator
        self.lifecycle.subscribe(self._on_lifecycle_change)

    def attach_orchestrator(self, orchestrator):
        """Install the canonical vNext execution boundary."""
        self.orchestrator = orchestrator
        return orchestrator

    async def execute(self, goal, task_id, metadata=None):
        """Execute a user goal through the canonical vNext orchestrator."""
        if self.orchestrator is None:
            raise RuntimeError("vNext orchestrator is not configured")
        return await self.orchestrator.run(goal, task_id, metadata=metadata)

    def _on_lifecycle_change(self, previous, current):
        self.events.publish(KernelEvent(name="lifecycle.changed", source="kernel.lifecycle", payload={"from": previous, "to": current}))
    ...
    def initialize(self):
        ...
        self.events.publish(KernelEvent(name="kernel.initialized", source="kernel", payload={"status": self.state.status, "valid": result["valid"]}))
        return result
    ...
    def start(self):
        ...
        self.events.publish(KernelEvent(name="kernel.started", source="kernel", payload={"status": self.state.status}))

    def stop(self):
        self.components.stop_all()
        self.lifecycle.transition(LifecyclePhase.STOP)
        self.state.status = RuntimeStatus.STOPPED
        self.events.publish(KernelEvent(name="kernel.stopped", source="kernel", payload={"status": self.state.status}))

    def register(self, name, component):
        self.registry.register(name, component)
        self.events.publish(KernelEvent(name="component.registered", source="kernel.registry", payload={"name": name, "component": component}))
