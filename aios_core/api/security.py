...
class APIKeyAuthMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, *, enabled: bool = True, api_keys: dict[str, Principal] | None = None):
        ...
        if api_keys is None:
            ...
            self.api_keys = {
                key: value if isinstance(value, Principal) else Principal(
                    subject=value["subject"], roles=frozenset(value.get("roles", []))
                )
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        supplied = header[7:]
        principal = next((p for key, p in self.api_keys.items() if hmac.compare_digest(key, supplied)), None)
