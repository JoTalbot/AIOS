...
def resolve_card_parser(platform_name: str, directory: str = "platforms") -> None:
    ...
    try:
        ...
        raise ValueError(
            f"no card parser for '{platform_name}': run 'aios platforms bootup ...' or calibrate --write ({exc})"
        ) from None
...
def autowatch_cycle(
    ...
    if queries is None:
        queries = sorted({sub.get("query") for sub in SubscriptionManager(storage).list() if sub.get("query")})
