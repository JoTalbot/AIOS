"""Очередь черновиков ответов Viber с явным подтверждением владельца.

Фоновый Viber-обработчик может сгенерировать текст, но не отправляет его сам в
...
class ViberDraftStore:
    def __init__(self, root: Path | str | None = None):
        self.root = Path(root) if root is not None else PROJECT_ROOT
        self.path = self.root / "data" / "viber_pending_replies.json"
        self.lock_path = self.root / "data" / ".viber_pending_replies.lock"
