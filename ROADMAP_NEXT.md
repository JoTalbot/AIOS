# AIOS Roadmap — Next Milestones (актуализировано 2026-08-17)

> Текущая версия: **v19.9.0** | 2026-08-17: полный pytest зелёный, 0 failed-сервисов,
> Docker Build & Push success, 10 PR смержены в main.

## Актуально на 2026-08-17 (сессия трейдинг-харденинга)

**Сделано:**
- Трейдинг-контур отхарднен: диск 94%→56%, retention orderbook-данных (7д/60д),
  busy_timeout коллекторов, честная latency (p90 3.1с→0.13с), MemoryMax, swap 8G.
- ML-гейт Directional v2 откалиброван (0.5061) — A/B paper торгует; интроспекция
  сделок; weekly A/B-отчёт + алерты в health check; calibration в deploy-гейте ретрайна.
- Исследования (честные негативы): MM (сигнал и очередь — нет edge), BTC
  long-horizon (утечка лейблов), maker-rebate (нет доступных ребейтов), T2
  hyperopt 56/87 отклонён на OOS.
- Scoreboard стратегий (ежемесячный автоматический бэктест-вердикт).
- Зависимости: runtime-бампы + lock по контракту; GHA-бампы; Trivy-исключения
  для Prometheus (снять при v3.14.0+); 14 dependabot-веток разобраны.

**Следующие вехи (по приоритету):**
1. Вердикт A/B Directional v2 — при ≥15 закрытых сделок в каждом контуре
   (бустрап-чекер уже в weekly-отчёте).
2. MM-вердикт — после 2-4 недель 1Гц-данных (приоритет очереди на 100мс-стриме).
3. При релизе Prometheus v3.14.0 — обновить пин, убрать trivy-исключения.
...
- ✅ **Stitch Skills (Google)**: 15 скилов (design/build/utilities) из google-labs-code/stitch-skills → skills/stitch/, клиент REST aios_core/stitch_client.py (STITCH_API_KEY, list/get/upload/export), проект «Universal Messenger Hub» (50 экранов) + новый «AIOS Warehouse Catalog» с каталогом склада
