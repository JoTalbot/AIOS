...
def test_tab_driver_no_dump_is_false():
    driver = ReelsTabDriver(adb=_ADB([]), open_wait_s=0,
...
def test_reels_driver_for_yaml_and_defaults(tmp_path):
    (tmp_path / "instagram.yaml").write_text(yaml.safe_dump({
        "name": "instagram",
        "android_package": "com.instagram.android",
        "extras": {"parser_hints": {"navigation": {"reels_tab": {
            "rid_markers": [
...
def test_tab_driver_failure_blocks_cycle():
    collector = ReelsCollector(
        _descriptor(), adb=_ADB([HOME_NO_TAB_XML]),
...
def test_video_new_alert_on_new_cards_only():
    ...
    notifier = _Notifier()
    collector = ReelsCollector(
        _descriptor(), adb=_ADB([_reels("N1", "N2"), EMPTY_XML]),
        parser=HintVideoParser(["reel_card"]), notifier=notifier,
    )
...
def test_cli_reels_open_tab_and_webhook(tmp_path, capsys, monkeypatch,
    ...
    monkeypatch.setattr(
        "aios_core.platforms.reelscout.time.sleep", lambda s: None,
    )
    main(["instagram", "reels", "--db", str(db), "--max", "10",
          "--directory", str(tmp_path), "--open-tab",
          "--webhook", "http://hook.example/in"])
...
def test_cli_reels_open_tab_missing_tab_reports_error(tmp_path, capsys,
                                                      monkeypatch,
                                                      instagram_registered):
    from aios_cli import main
    ...
    main(["instagram", "reels", "--db", str(tmp_path / "ig.sqlite"),
          "--directory", str(tmp_path), "--open-tab"])
    out = json.loads(capsys.readouterr().out)
    assert "error" in out and "видео-ленту" in out["error"]


def test_cli_autopilot_open_tab_and_webhook(tmp_path, capsys, monkeypatch,
                                            instagram_registered):
    from aios_cli import main
...
def _seed_profiles(tmp_path, monkeypatch, *pairs):
    ...
    for platform, name in pairs:
        store.add(Profile(platform=platform, name=name, is_default=True,
...
def test_cron_plan_shard_map_groups_by_host(tmp_path, capsys, monkeypatch):
    ...
    _seed_profiles(
        ...
        ("instagram", "main"), ("instagram", "backup"), ("tiktok", "fun"),
    )
    ...
    header = next(
        lines[i] for i in range(auto_idx, -1, -1)
...
def test_cron_plan_shard_map_no_hosts_local_group(tmp_path, capsys,
                                                  monkeypatch):
    from aios_cli import main
