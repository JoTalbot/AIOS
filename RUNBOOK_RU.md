...
### Рынок ВАЗ/ГАЗель, выкуп и фотокаталог (2026-08-05)
- Коллектор OLX: только ниши авторазборок и б/у запчастей ВАЗ/ГАЗель
  (9 запросов, RU+UA); старые общие запросы удалены, ads-база вычищена.
- run_market_digest.py: дневные снапшоты медиан (data/market_snapshots.json,
  таймер 03:00) + блок «Рынок ВАЗ/ГАЗель» в утреннем брифинге.
- run_olx_price_alerts.py: алерт при падении min-цены ниши >5%
  (подписки data/olx_price_subs.json).
- run_deal_scanner.py (30 мин): новые лоты ≤50% медианы — «🔥 Выгодный лот»,
  доноры под разбор ВАЗ/ГАЗель — «🚙 ДОНОР»; суммарно ≤3 алертов/сутки.
- Фотокаталог: run_stock_photos.py pull [--item] — фото из DCIM телефона,
