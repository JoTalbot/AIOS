...
class Planner:
    ...
    def _llm_json(self, prompt: str) -> dict:
        system = (
            ...
            "СБОР ДАННЫХ КЛИЕНТА (ОБЯЗАТЕЛЬНО ВСЕГДА): "
            "Для успешного взаимодействия и отправки ВСЕГДА заранее выясняй у клиента полные данные "
            "для доставки. Для отправки НОВОЙ ПОШТОЙ нужны ТОЛЬКО: город, номер отделения, "
            "ФИО (имя и фамилия), номер телефона получателя. АДРЕС (улица/дом) НЕ нужен — не спрашивай. "
            "Спрашивай эти данные уже при первом контакте/интересе к товару, а не только при "
            "оформлении покупки. Если данных нет — в params.text задай вежливый вопрос клиенту."
        )
    ...
    def _propose_customer(self, platform: str, chat: str, text: str, intent: dict,
        ...
        # Контекст сделки — чтобы LLM НЕ забывал уже согласованные детали (цена/товар/доставка/данные)
        sale_block = ""
        sc = extra.get("sale_context")
        if sc:
            sale_block = "КОНТЕКСТ СДЕЛКИ (уже согласовано с этим клиентом, помни это):\n" + str(sc) + "\n"
        prompt = (
            ...
            "params.phone (телефон клиента, если указал), params.recipient (ФИО, если указал) "
            "и params.text (подтверждение клиенту). Это фиксирует сделку для подготовки ТТН.\n"
            "ДАННЫЕ ДЛЯ ОТПРАВКИ (Нова Пошта): для отправки нужны ТОЛЬКО город, номер отделения, "
            "ФИО (имя/фамилия), номер телефона. АДРЕС не нужен. Если этих данных нет — задай "
            "вежливый вопрос клиенту в params.text, чтобы получить их СЕЙЧАС.\n"
