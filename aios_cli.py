...
def _add_all_subparsers(subparsers):
    ...
    p.add_argument("--locale", default="uk-UA")
    p.add_argument("--root", default=".")
    p.add_argument("--dry-run", action="store_true")
    p = ps.add_parser("from-apk")
    p.add_argument("apk")
    p.add_argument("--name", default=None)
    p.add_argument("--locale", default="uk-UA")
    p.add_argument("--root", default=".")
    p.add_argument("--dry-run", action="store_true")
    p = ps.add_parser("calibrate")
    p.add_argument("--platform", required=True)
    p.add_argument("--dump", required=True)
    p.add_argument("--detail", default=None)
    p.add_argument("--messages", default=None)
    p.add_argument("--navigation", default=None)
    p.add_argument("--write", action="store_true")
    p.add_argument("--root", default=".")
    p = ps.add_parser("reels")
    p.add_argument("--platform", required=True)
    p.add_argument("--profile", default=None)
    p.add_argument("--db", default=None)
    p.add_argument("--max", type=int, default=100, dest="max_cards")
    p.add_argument("--serial", default=None)
    p.add_argument("--directory", default="platforms")
    p.add_argument("--open-tab", action="store_true")
    ...
    p = ps.add_parser("fetch-apk")
    p.add_argument("package")
    p.add_argument("--out", default="apks")
    p.add_argument("--source", default="apkpure")
    p = ps.add_parser("autowatch")
    p.add_argument("--platform", required=True)
    p.add_argument("--profile", default=None)
    ...
    p = ps.add_parser("marker-check")
    p.add_argument("--platform", required=True)
    p.add_argument("--dump", required=True)
    p = ps.add_parser("codegen")
    p.add_argument("--platform", required=True)
    p.add_argument("--root", default=".")
    p.add_argument("--dry-run", action="store_true")
    p.add_argument("--force", action="store_true")
    p = ps.add_parser("bootup")
    ...
    p.add_argument("--apks-dir", default="apks")
    p.add_argument("--serial", default=None)
    ...
    # Onboard
    p = subparsers.add_parser("onboard")
    ...
    for app in ("whatsapp", "viber", "facebook"):
        ...
        for cmd in ("chats", "dm-send", "dm-flush", "dm-outbox"):
            ...
            if cmd == "dm-send":
                ...
                c.add_argument("--auto-send", action="store_true")
            c.add_argument("--db", default=None)
            c.add_argument("--serial", default=None)
            c.add_argument("--directory", default="platforms")
    ...
    for cmd, args in [
        ("collect", [("--query", None), ("--max", 100)]),
        ("dm-send", [("--chat", True), ("--text", True), ("--auto-send", False)]),
        ("dm-flush", []),
        ("dm-outbox", []),
        ("login-drive", [("--query", None)]),
        ("own", [("--dump", None)]),
        ("post", [("--image", True), ("--text", True), ("--confirm", False)]),
        ("reels", [("--max", 100), ("--open-tab", False), ("--webhook", None)]),
        (
            "autopilot",
            [
                ("--query", None),
        ...
        for an, av in args:
            c.add_argument(
                an, type=int if isinstance(av, int) and not isinstance(av, bool) else None, default=av
            ) if av is not False else c.add_argument(an, action="store_true")
    for cmd in ["collect", "dm-send", "dm-flush", "dm-outbox", "own", "reels", "autopilot", "post"]:
        ...
        if cmd in igs.choices:
            igs.choices[cmd].add_argument("--serial", default=None)
            igs.choices[cmd].add_argument("--directory", default="platforms")

    # Profiles
    prof = subparsers.add_parser("profiles")
    ps2 = prof.add_subparsers(dest="profiles_command")
    p = ps2.add_parser("list")
    p.add_argument("--platform", default=None)
    p = ps2.add_parser("add")
    p.add_argument("--platform", required=True)
    p.add_argument("--name", required=True)
    p.add_argument("--device", default=None)
    p.add_argument("--db", default=None)
    p.add_argument("--android-user", type=int, default=0)
    p.add_argument("--locale", default="uk-UA")
    ...
    # Shards
    sh = subparsers.add_parser("shards")
    ss = sh.add_subparsers(dest="shards_command")
    ss.add_parser("list")
    p = ss.add_parser("add")
    p.add_argument("--host", required=True)
    p.add_argument("--base-url", required=True)
    p = ss.add_parser("remove")
    p.add_argument("--host", required=True)
    for c in ("route", "unroute"):
        p = ss.add_parser(c)
        p.add_argument("--profile", required=True)
    p = ss.add_parser("monitor")
    p.add_argument("--interval", type=float, default=30.0)
    p.add_argument("--once", action="store_true")
    p = ss.add_parser("jobs")
    p.add_argument("--status", default=None)
    p.add_argument("--stats", action="store_true")
    p.add_argument("--ttl", type=float, default=600)
    p = ss.add_parser("requeue-stale")
    p.add_argument("--ttl", type=float, default=600)
    p = ss.add_parser("enqueue")
    p.add_argument("--profile", required=True)
    p.add_argument("--kind", required=True)
    p.add_argument("--payload", default=None)
    p = ss.add_parser("work")
    p.add_argument("--host", required=True)
    p.add_argument("--once", action="store_true")
    p.add_argument("--interval", type=float, default=30.0)

    # Cron-plan
    p = subparsers.add_parser("cron-plan")
    p.add_argument("--platform", default="olx")
    p.add_argument("--interval", type=int, default=15)
    p.add_argument("--webhook", default=None)
    p.add_argument("--write", default=None)
    p.add_argument("--with-marker-check", action="store_true")
    p.add_argument("--shard-map", action="store_true")
    p.add_argument("--via-shards", action="store_true")

    # Devices
    dev = subparsers.add_parser("devices")
    ds = dev.add_subparsers(dest="devices_command")
    ds.add_parser("list")
    p = ds.add_parser("register")
    p.add_argument("--serial", required=True)
    p.add_argument("--avd", default=None)
    p = ds.add_parser("lease")
    p.add_argument("--profile", required=True)
    p.add_argument("--serial", default=None)
    p.add_argument("--enqueue", action="store_true")
    p.add_argument("--priority", type=int, default=0)
    p.add_argument("--sync", action="store_true")
    ds.add_parser("waitlist")
    p = ds.add_parser("enqueue")
    p.add_argument("--profile", required=True)
    p.add_argument("--priority", type=int, default=0)
    p = ds.add_parser("cancel-wait")
    p.add_argument("--profile", required=True)
    p = ds.add_parser("release")
    p.add_argument("--profile", required=True)
    p = ds.add_parser("heartbeat")
    p.add_argument("--serial", required=True)
    p = ds.add_parser("reap")
    p.add_argument("--max-silence-s", type=float, default=900.0)
    p = ds.add_parser("ensure")
    p.add_argument("--profile", required=True)
    p.add_argument("--avd-prefix", default="aios")
    p = ds.add_parser("monitor")
    p.add_argument("--interval", type=float, default=30.0)
    p.add_argument("--once", action="store_true")
    p.add_argument("--reap-after-s", type=float, default=900.0)
    p = ds.add_parser("fleet-run")
    p.add_argument("--every-s", type=float, default=900.0)
    p.add_argument("--query", action="append", default=[])
    p.add_argument("--webhook", default=None)
    p = ds.add_parser("limits")
    p.add_argument("--set", dest="set_limit", default=None, metavar="KEY=VALUE")

    # Admin
    adm = subparsers.add_parser("admin")
    adms = adm.add_subparsers(dest="admin_command")
    p = adms.add_parser("export")
    p.add_argument("--type", choices=["tasks", "memory", "audit", "knowledge", "all"], default="all")
    p.add_argument("--format", choices=["json", "csv"], default="json")
    p.add_argument("--output", "-o", default="./export")
    p.add_argument("--since", default=None)
    p.add_argument("--limit", type=int, default=None)
    p.add_argument("--db", default="aios.sqlite")
    p = adms.add_parser("import")
    p.add_argument("--type", choices=["tasks"], default="tasks")
    p.add_argument("--format", choices=["json", "csv"], default="json")
    p.add_argument("--input", "-i", required=True)
    p.add_argument("--db", default="aios.sqlite")
    keys = adms.add_parser("keys")
    ks = keys.add_subparsers(dest="keys_command")
    p = ks.add_parser("generate")
    p.add_argument("--subject", required=True)
    p.add_argument("--roles", nargs="+", default=["viewer"])
    p.add_argument("--ttl", type=int, default=None)
    p.add_argument("--prefix", default="aios")
    p = ks.add_parser("list")
    p.add_argument("--subject", default=None)
    p = ks.add_parser("revoke")
    p.add_argument("--key", required=True)
    p.add_argument("--reason", default="")
    p = ks.add_parser("rotate")
    p.add_argument("--key", required=True)
    p.add_argument("--ttl", type=int, default=None)
    p.add_argument("--reason", default="rotation")
    ks.add_parser("health")
    bu = adms.add_parser("backup")
    bs = bu.add_subparsers(dest="backup_command")
    for cmd, extra_args in [
        ("create", [("--label", ""), ("--mode", "full")]),
        ("list", []),
        ("verify", [("--backup-id", True)]),
        ("restore", [("--backup-id", True), ("--target", None)]),
        ("cleanup", []),
        ("health", []),
    ]:
        p = bs.add_parser(cmd)
        for an, av in extra_args:
            if av is True:
                ...
                p.add_argument(an, default=av)
        p.add_argument("--db", default="aios.sqlite")
        p.add_argument("--backup-dir", default="./backups")
    wh = adms.add_parser("webhooks")
    ws = wh.add_subparsers(dest="webhooks_command")
    p = ws.add_parser("register")
    p.add_argument("--name", required=True)
    p.add_argument("--url", required=True)
    p.add_argument("--events", nargs="+", required=True)
    p.add_argument("--secret", default=None)
    ws.add_parser("list")
    p = ws.add_parser("test")
    p.add_argument("--name", required=True)
    p = ws.add_parser("notify")
...
def main(argv=None):
    ...
    if args.command == "run":
        ...
        if not _run_msg_platform(args, args.command):
        ...
        try:
            handled = _run_rozetka(args)
        except ValueError as exc:
            print(json.dumps({"error": str(exc)}, ensure_ascii=False))
            handled = True
        if not handled:
            parser.parse_args(["rozetka", "--help"])
    elif args.command == "cross-platform":
        try:
            handled = _run_cross_platform(args)
        except ValueError as exc:
            print(json.dumps({"error": str(exc)}, ensure_ascii=False))
            handled = True
        if not handled:
            parser.parse_args(["cross-platform", "--help"])
    elif args.command == "advisor-v2":
        try:
            handled = _run_advisor_v2(args)
        except ValueError as exc:
            print(json.dumps({"error": str(exc)}, ensure_ascii=False))
            handled = True
        if not handled:
            parser.parse_args(["advisor-v2", "--help"])
    elif args.command == "search":
        try:
            handled = _run_search(args)
        except ValueError as exc:
            print(json.dumps({"error": str(exc)}, ensure_ascii=False))
            handled = True
        if not handled:
        ...
        try:
            handled = _run_benchmarks(args)
        except ValueError as exc:
            print(json.dumps({"error": str(exc)}, ensure_ascii=False))
            handled = True
        if not handled:
            parser.parse_args(["benchmarks", "--help"])
    elif args.command == "tiktok-shop":
        try:
            handled = _run_tiktok(args)
        except ValueError as exc:
            print(json.dumps({"error": str(exc)}, ensure_ascii=False))
            handled = True
        if not handled:
            parser.parse_args(["tiktok-shop", "--help"])
    elif args.command == "fb-marketplace":
        try:
            handled = _run_facebook(args)
        except ValueError as exc:
            print(json.dumps({"error": str(exc)}, ensure_ascii=False))
            handled = True
        if not handled:
            parser.parse_args(["fb-marketplace", "--help"])
    elif args.command == "whatsapp-v2":
        try:
            handled = _run_whatsapp_v2(args)
        except ValueError as exc:
            print(json.dumps({"error": str(exc)}, ensure_ascii=False))
            handled = True
        if not handled:
            parser.parse_args(["whatsapp-v2", "--help"])
    elif args.command == "viber-v2":
        try:
            handled = _run_viber_v2(args)
        except ValueError as exc:
            print(json.dumps({"error": str(exc)}, ensure_ascii=False))
            handled = True
        if not handled:
            parser.parse_args(["viber-v2", "--help"])
    elif args.command == "price-predict":
        try:
            handled = _run_price_predict(args)
        except ValueError as exc:
            print(json.dumps({"error": str(exc)}, ensure_ascii=False))
            handled = True
        if not handled:
            parser.parse_args(["price-predict", "--help"])
    elif args.command == "image-compare":
        try:
            handled = _run_image_compare(args)
        except ValueError as exc:
            print(json.dumps({"error": str(exc)}, ensure_ascii=False))
            handled = True
        if not handled:
            parser.parse_args(["image-compare", "--help"])
    elif args.command == "fleet":
        try:
            handled = _run_fleet(args)
        except ValueError as exc:
            print(json.dumps({"error": str(exc)}, ensure_ascii=False))
            handled = True
        if not handled:
        ...
        cmd_map = {
            ...
            "import": run_import,
            "keys": lambda a: {
                "generate": run_keys_generate,
                "list": run_keys_list,
                "revoke": run_keys_revoke,
                "rotate": run_keys_rotate,
                "health": run_keys_health,
            }.get(getattr(a, "keys_command", ""), lambda x: parser.parse_args(["admin", "keys", "--help"]))(a),
            "backup": lambda a: {
                "create": run_backup_create,
                "list": run_backup_list,
                "verify": run_backup_verify,
                "restore": run_backup_restore,
                "cleanup": run_backup_cleanup,
                "health": run_backup_health,
            }.get(getattr(a, "backup_command", ""), lambda x: parser.parse_args(["admin", "backup", "--help"]))(a),
            "webhooks": lambda a: {
                "register": run_webhooks_register,
                "list": run_webhooks_list,
                "test": run_webhooks_test,
                "notify": run_webhooks_notify,
