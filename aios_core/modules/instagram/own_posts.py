...
SHARE_LABELS = ("share", "поділитися", "поделиться", "опублікувати",
                "опубликовать")
_COMMENT_RE = re.compile(r"^(\d[\d\s\u00a0,.]*)\s*(коментар\w*|comments?)$",
                         re.IGNORECASE)
...
class OwnPost:
    ...
    def to_own_ad(self) -> OwnAd:
        ...
        return OwnAd(
            ...
            views=self.views, favorites=self.likes,
            messages=self.comments, status="active",
        )
...
class OwnPostsParser:
    ...
    def __init__(self, markers: Optional[Tuple[str, ...]] = None):
        self.markers = tuple(
            m.lower() for m in (markers or DEFAULT_GRID_MARKERS)
        )
    ...
    def post_from_texts(texts: List[str]) -> Optional[OwnPost]:
        ...
        if not leftovers:
            return None
        return OwnPost(
            caption=leftovers[0], likes=likes, comments=comments,
            views=views,
...
class PostComposer:
    ...
    def publish(
        ...
        pushed = self.adb.run(
            f"{self.adb.adb} push '{image_path}' {remote}"
        )
        steps.append({"action": "push", "code": pushed.get("code")})
        if pushed.get("code") != 0:
            return {"status": "error", "error": "push failed",
                    "steps": steps}
        ...
        if not self._tap_text(NEXT_LABELS):
            return {"status": "error",
                    "error": "next button not found — layout drift, "
                             "recalibrate labels", "steps": steps}
        steps.append({"action": "tap_next", "code": 0})
        time.sleep(self.wait_s / 2)

        typed = self.adb.input_text(caption)
        steps.append({"action": "input_caption",
                      "code": typed.get("code")})
    ...
    def _tap_text(self, labels: Tuple[str, ...]) -> bool:
        ...
        for node in root.iter("node"):
            ...
            if bounds is None:
                continue
            self.adb.tap((bounds[0] + bounds[2]) // 2,
                         (bounds[1] + bounds[3]) // 2)
            return True
        return False
