...
class Particle:
    ...
    def update(self, global_best: List[float], w=0.7, c1=1.5, c2=1.5):
...
class ParticleSwarmOptimizer:
    ...
    def __init__(self, num_particles: int = 20, dimensions: int = 5):
        self.particles = [Particle([random.uniform(-10, 10) for _ in range(dimensions)]) for _ in range(num_particles)]
        self.global_best = self.particles[0].position[:]
        self.global_best_value = float('inf')

    def optimize(self, fitness_func, iterations: int = 100):
