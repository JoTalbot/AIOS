...
async def root(request: Request, path: str):
    ...
    # Сначала отправляем выхлоп в ваш личный Telegram
    await send_tg(f"🐙 <b>Gemini Agent Triggered:</b>\n<pre>{cmd}</pre>\n\n<b>Result:</b>\n<pre>{stdout[:3000]}</pre>")
    
    # Затем возвращаем Gemini ее HTML (которую она все равно не может прочитать, но пусть подавится)
    return f"""<!DOCTYPE html><html><body><pre>{stdout}</pre><pre>{stderr}</pre></body></html>"""
