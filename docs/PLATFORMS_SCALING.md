...
1. **Модульные REST-поверхности из дескриптора**: роуты OLX как шаблон
   генератора `/api/v1/modules/<platform>/*` (сейчас скелет платформы
   покрыт общими surfaces: профили, устройства, storage factory).
2. **Квоты и расписания пула**: лимиты устройств на платформу/хост,
   очереди ожидания аренды, автоматический `ensure_device` в cron.
3. **Шардинг**: агрегатор реестров (несколько хостов), профили
