...
def test_render_contains_core_series():
    ...
    text = render_prometheus(memory_system=memory, engine=engine, scheduler=scheduler, version="15.0.0")
    assert "# HELP aios_info AIOS build information (constant 1)." in text
    assert "# TYPE aios_info gauge" in text
    assert 'aios_info{version="15.0.0"} 1' in text
    # Memory block
...
def test_metrics_endpoint_shape(client):
    ...
    body = resp.text
    assert 'aios_info{version="15.0.0"} 1' in body
