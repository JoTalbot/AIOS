...
class TestScheduledTask:
    def test_is_recurring_false(self) -> None:
        t = ScheduledTask(name="one", func=lambda: 1, run_at=datetime.now(timezone.utc))
        assert t.is_recurring() is False

    def test_is_recurring_true(self) -> None:
        t = ScheduledTask(
            name="rec", func=lambda: 1, run_at=datetime.now(timezone.utc), recurring_interval=timedelta(seconds=60)
        )
        assert t.is_recurring() is True

    def test_can_retry(self) -> None:
        t = ScheduledTask(name="r", func=lambda: 1, run_at=datetime.now(timezone.utc), max_retries=3)
        assert t.can_retry() is True
        t.retry_count = 3
        assert t.can_retry() is False

    def test_next_run_time(self) -> None:
        t = ScheduledTask(name="r", func=lambda: 1, run_at=datetime.now(timezone.utc), recurring_interval=timedelta(seconds=10))
...
class TestTaskScheduler:
    ...
    def test_schedule(self) -> None:
        t = self.ts.schedule("task1", lambda: 1, datetime.now(timezone.utc) + timedelta(seconds=1))
        assert t.name == "task1"
    ...
    def test_schedule_with_priority(self) -> None:
        t = self.ts.schedule_with_priority("p_task", lambda: 1, datetime.now(timezone.utc), priority=TaskPriority.CRITICAL)
        assert t.priority == TaskPriority.CRITICAL

    def test_schedule_with_retry(self) -> None:
        t = self.ts.schedule_with_retry("r_task", lambda: 1, datetime.now(timezone.utc), max_retries=3)
        assert t.max_retries == 3

    def test_tick_executes_due(self) -> None:
        results = []
        self.ts.schedule("now", lambda: results.append("executed"), datetime.now(timezone.utc) - timedelta(seconds=1))
        self.ts.tick()
        assert "executed" in results

    def test_tick_skips_future(self) -> None:
        self.ts.schedule("future", lambda: 1, datetime.now(timezone.utc) + timedelta(hours=1))
    ...
    def test_tick_priority_order(self) -> None:
        order = []
        self.ts.schedule(
            "low", lambda: order.append("low"), datetime.now(timezone.utc) - timedelta(seconds=1), priority=TaskPriority.LOW
        )
        self.ts.schedule(
            "crit",
    ...
    def test_tick_failure(self) -> None:
        self.ts.schedule(
            "fail", lambda: (_ for _ in ()).throw(ValueError("err")), datetime.now(timezone.utc) - timedelta(seconds=1)
        )
        self.ts.tick()
    ...
    def test_tick_recurring_reschedule(self) -> None:
        ...
        self.ts.tasks["rec"].run_at = datetime.now(timezone.utc) - timedelta(seconds=1)
        self.ts.tick()
        task = self.ts.get_task("rec")
        assert task.status == TaskScheduleStatus.SCHEDULED  # rescheduled

    def test_cancel(self) -> None:
        self.ts.schedule("t1", lambda: 1, datetime.now(timezone.utc) + timedelta(seconds=10))
        self.ts.cancel("t1")
    ...
    def test_cancel_all(self) -> None:
        self.ts.schedule("t1", lambda: 1, datetime.now(timezone.utc))
        self.ts.schedule("t2", lambda: 1, datetime.now(timezone.utc))
    ...
    def test_get_completed(self) -> None:
        self.ts.schedule("t1", lambda: 1, datetime.now(timezone.utc) - timedelta(seconds=1))
        self.ts.tick()
...
class TestIntegration:
    def test_scheduler_with_event_store(self) -> None:
        ...
        es = EventStore()
        events_log = []
        ts.schedule("log_event", lambda: events_log.append("done"), datetime.now(timezone.utc) - timedelta(seconds=1))
    ...
    def test_observability_with_task_scheduler(self) -> None:
        ...
        ts.schedule("task1", lambda: 1, datetime.now(timezone.utc) - timedelta(seconds=1))
        ts.tick()
