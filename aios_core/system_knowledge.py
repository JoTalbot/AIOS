...
Собирает из реальных источников репозитория:
  ...
  * курируемый список NL-команд и команд Telegram-бота
...
def _load_adapters() -> list[tuple[str, str]]:
    ...
    for f in sorted(plat.glob("*adapter*.py")) + sorted(plat.glob("*chrome_twin*.py")):
        ...
        for m in re.finditer(r"^class\s+(\w+)\s*(?:\([^)]*\))?:\s*$", text, re.M):
            cls = m.group(1)
            # первая строка docstring сразу после def/class
...
# ---------------------------------------------------------------------------
# Курируемые NL-команды (основные фразы управления системой)
# ---------------------------------------------------------------------------

NL_COMMANDS: list[tuple[str, str]] = [
    # Google
    ("Google-аккаунт", "«проверь почту», «непрочитанные», «покажи календарь», «события на неделю», «диск», «скачай файл …», «создай документ …», «отправь письмо адрес, тема …, текст: …» (с подтверждением), «найди письмо …», «ответь на письмо N: текст»"),
    # Instagram
    ("Instagram", "«мой инстаграм», «мои посты», «скрин профиля», «лайкни <url>», «подпишись на @…», «директ», «покажи чат <имя>», «напиши в директ <имя>: текст»"),
    # OLX
    ("OLX", "«создай объявление …», «опубликуй на олх …», «мои объявления», «подними объявления», «удали объявление …», «следи за ценой …», «мониторинг цен», «отчёт по OLX», «автоответ олх», «кто продаёт дешевле …»"),
    # Склад / инвентарь
    ("Склад", "«добавь деталь <название>, N шт по <цена>», «спиши деталь …», «что на складе», «найди деталь …», «инвентаризация», «сколько стоит …», «остатки»"),
    # Финансы
    ("Финансы", "«запиши продажу …» / «запиши расход …», «сколько заработал», «прибыль», «мои операции», «деньги за неделю/месяц», «месячный отчёт», «финансы в таблицу»"),
    # Новая Почта / логистика
    ("Новая Почта", "«создай ТТН <деталь>, <стоимость>, <получатель>, <телефон>, <город>, <отделение>» (с подтверждением), «отследи <ТТН>», «клиенты», «отправил <ТТН>», «посылки»"),
    # Единый инбокс
    ("Инбокс", "«инбокс», «все сообщения», «где что новое», «проверь всё», «озвучь инбокс», «найди во всех чатах <запрос>», «ответь на N: текст», «отметь всё прочитанным», «присылай инбокс в HH:MM»"),
    # Напоминания
    ("Напоминания", "«напомни [завтра/сегодня] в HH:MM <текст>», «напоминай каждый день в 09:00 …»"),
    # Телефон / Android
    ("Телефон", "«мозг» (статус Phone Brain), «черновики», «подтверди N», «центр телефона», «восстановление телефона», «инвентарь телефона», «банки телефона», «маршруты», «калибровки», «журнал телефона»"),
    # Фото / распознавание
    ("Фото", "«распознай деталь» (по фото), «сделай объявление из фото», «оцени деталь», «что за деталь»"),
    # Сервер
    ("Сервер", "«покажи логи», «статус сервера», «docker ps» через /cmd, «перезапусти <сервис>» через /cmd, «/cmd <команда>» — прямой доступ к консоли"),
]
...
def _prompt_text(s: dict[str, Any]) -> str:
    """Компактная справка для промпта LLM (~2-3 КБ)."""
    lines = []
    ...
    lines.append("")
    lines.append("УПРАВЛЕНИЕ ОБЫЧНЫМ ЯЗЫКОМ (напиши боту фразу):")
    ...
    lines.append("«как сделать X» — используй эту справку и предложи конкретную команду/фразу.")
    return "\n".join(lines)
