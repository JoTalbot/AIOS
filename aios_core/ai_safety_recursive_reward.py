...
class RecursiveRewardModel:
        ...
        self._feedback_log.append(
            {
                "model": new_model,
                "feedback_type": "human",
                "count": len(human_feedback) if isinstance(human_feedback, (list, dict)) else 1,
            }
        )
    ...
    def stack_models(self, models: list[str]) -> str:
        ...
        self.reward_models[stacked_name] = lambda x: (
            sum(self.reward_models.get(m, lambda _: 0.5)(x) for m in models) / len(models)
        )
    ...
    def convergence_check(self, base_model: str) -> bool:
        ...
        quality_changes = [
            abs(recent_entries[i].quality - recent_entries[i - 1].quality) for i in range(1, len(recent_entries))
        ]
