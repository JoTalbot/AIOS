...
### 🚀 Milestone 4.2.1: Advanced ML Intelligence Layer (Недели 1–3)
...
- **Критерии приемки:**
  - Поддержка сериализации `.onnx` и `.pkl`.
  - Метод `predict(model_id, input_data)` со средней задержкой < 10 мс.
  - Версионирование моделей и откат на резервные веса.
...
- **Критерии приемки:**
  - Алгоритмы Isolation Forest / One-Class SVM на статистике метрик.
  - Генерация события `EventType.ANOMALY_DETECTED` при превышении порога.
...
- **Модули:** `aios_core/autonomy_manager.py`
- **Критерии приемки:**
  - Временное понижение уровня автономии агента при высоком прогнозируемом риске.

#### WBS 1.4 — Покрытие Тестами ML Слой
- **Файлы тестов:** `tests/test_ml_registry.py`, `tests/test_anomaly_detection.py`
