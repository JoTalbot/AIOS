...
def test_dag_self_healing():
    ...
    step1 = WorkflowStep(id="step1", name="fetch", action=fail_action, is_self_healing=True, healing_action=heal_action)

    def step2_action(step1_result, **kwargs):
        assert step1_result["data"] == "fallback_data"
        return "success"

    step2 = WorkflowStep(id="step2", name="process", action=step2_action, depends_on=["step1"])
