...
class SubscriptionManager:
    ...
    def matches(subscription: dict[str, object], card: AdCard) -> bool:
        ...
        same_query = (card.query or "").strip().lower() == (subscription["query"] or "").strip().lower()
        if not same_query:
    ...
    def check_new(self, new_cards: builtins.list[AdCard]) -> builtins.list[dict[str, object]]:
...
class FavoritesWatch:
    ...
    def price_alerts(self) -> builtins.list[dict[str, object]]:
