...
def load_series(path: Path, gran: str = "day") -> dict[int, float]:
    """Load ts->value; normalize ts to day or hour so series align.

    Daily series (Yahoo close times vary) -> UTC midnight.
    Hourly series -> top of hour.
    """
    step = 86400 if gran == "day" else 3600
    out = {}
    if not path.exists():
        return out
    for line in path.read_text().splitlines():
        try:
            r = json.loads(line)
            out[(int(r["ts"]) // step) * step] = float(r["value"])
        except Exception:
            continue
    return out
...
def main() -> int:
    ...
    hprice = load_series(args.data_dir / "BTC_USD_1h.jsonl", gran="hour") if (args.data_dir / "BTC_USD_1h.jsonl").exists() else {}
