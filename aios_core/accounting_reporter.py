...
class AIOSAccountingReporter:
    ...
    def __init__(self, data_dir: str = "/root/AIOS/data"):
        ...
        in_container_env = os.path.exists('/.dockerenv')
        if not in_container_env and os.path.exists('/proc/self/cgroup'):
            with open('/proc/self/cgroup') as cgroup_file:
                in_container_env = 'docker' in cgroup_file.read()
        if in_container_env and os.path.exists("/app/data"):
            data_dir = "/app/data"
