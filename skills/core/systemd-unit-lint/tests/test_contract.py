#!/usr/bin/env python3
"""Contract tests for systemd-unit-lint skill."""
...
def test_run_py_not_generic_runtime():
    ...
    assert "lint_unit" in code or "parse_unit_file" in code
...
def test_run_produces_valid_json():
    ...
    assert data.get("skill") == "systemd-unit-lint"
    assert "units_found" in data
