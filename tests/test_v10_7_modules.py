...
class TestAdvancedSecurity:
    ...
    def test_custom_policy(self):
        self.sec.add_policy(
            SecurityPolicy(
                "block_admin", "admin_access", ThreatLevel.HIGH, check_fn=lambda r: r.get("path") == "/admin"
            )
        )
...
class TestAdversarialDefense:
    ...
    def test_apply_custom_strategy(self):
        self.ad.add_strategy(
            DefenseStrategy("smooth", AttackType.RANDOM_PERTURBATION, defense_fn=lambda d: [sum(d) / len(d) for _ in d])
        )
