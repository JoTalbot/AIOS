...
1. **Генерация поверхностей**: CLI-подкоманду `aios <platform>` и
   REST-маршруты `/api/v1/modules/<platform>/*` генерирует кодогенератор
   из дескриптора — единственный способ выдержать тысячи приложений без
   ручного кодирования. Роуты OLX — шаблон генератора.
2. **Агент bootstrap'а устройств**: связка `OLXBootstrap` × `DevicePool` —
   автоматическое создание AVD под новый профиль при нехватке
   устройств в пуле.
