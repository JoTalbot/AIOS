...
class CapabilityMarketplace:
    ...
    def publish(self, name: str, description: str, author: str = "system",
                tags: Optional[List[str]] = None, code: str = "",
                kind: str = "capability", metadata: Optional[Dict[str, Any]] = None) -> MarketplaceCapability:
        ...
        if self.db:
            try:
                self.db.execute(
                    ...
                    (item.id, item.name, item.description, item.author, item.version,
                     json.dumps(item.tags), item.downloads, item.rating, item.code_snippet,
                     item.kind, json.dumps(item.metadata))
                )
            except Exception:
                pass
        return item
    ...
    def publish_platform_plugin(self,
                                platform: str,
                                descriptor_yaml: str,
                                ...
                                author: str = "system",
                                readme: str = "",
                                version: str = "1.0.0") -> PlatformPlugin:
        ...
        if self.db:
            try:
                self.db.execute(
                    ...
                    (plugin.id, plugin.platform, plugin.descriptor_yaml,
                     json.dumps(plugin.hints), json.dumps(plugin.drivers),
                     plugin.version, plugin.author, int(plugin.verified),
                     plugin.downloads, plugin.readme, plugin.created_at)
                )
            except Exception:
                pass
        return plugin
    ...
    def stats(self) -> dict:
        return {
            ...
            "total_downloads": sum(i.downloads for i in self._items.values()) + sum(p.downloads for p in self._plugins.values()),
