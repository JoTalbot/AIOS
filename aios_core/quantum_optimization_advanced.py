...
class QuantumApproximateOptimization:
    ...
    def constrained_qaoa(
        ...
        penalty = sum(round(random.uniform(0.01, 0.5), 3) for _ in constraints) if constraints else 0.0
        return {
    ...
    def portfolio_optimization(self, assets: int = 5, budget: float = 1000.0) -> dict[str, Any]:
