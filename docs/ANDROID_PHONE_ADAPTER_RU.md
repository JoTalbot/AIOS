...
На телефоне установлен `AIOS Companion`. Его локальный gateway слушает только
во внутреннем WireGuard-канале и требует отдельный токен AIOS. После выданных
пользователем разрешений доступны:

- состояние батареи и сети;
- список приложений;
- уведомления через Notification Listener;
- геолокация, когда Android уже получил координаты;
- заготовка разрешений камеры и файлов.

Выбранные уведомления WhatsApp, A-Bank, Privat24, Uklon и iMe собираются в
общий инбокс через `aios-android-notifications.timer`. Одноразовые коды и
похожие короткие числа маскируются до попадания в инбокс.

Уведомления и местоположение не отправляются сами: AIOS получает их только по
явной команде владельца. Камера, SMS, звонки и контакты будут добавляться
отдельными подтверждаемыми возможностями, а не скрытым фоновым доступом.
