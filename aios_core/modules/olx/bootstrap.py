    ...
    def plan(
        ...
        steps.append(BootstrapStep(
            ...
            ([f"adb install -r {self.workdir}/ADBKeyboard.apk"]
             + ([f"adb install -r {olx_apk}"] if olx_apk else
        ...
        for step in self.plan(**kwargs):
            for command in step.commands:
                ...
                if outcome.get("code") not in (0, None):
                    results.append({
                        "step": step.name, "command": command,
                        "code": outcome.get("code"), "aborted": True,
                    })
                    return results
        return results
    ...
    def doctor(self, db_path: str = ":memory:") -> List[DoctorCheck]:
        ...
        if adb_ok:
            ...
            checks.append(DoctorCheck(
                ...
                "" if online else "Підключіть телефон або запустіть емулятор "
